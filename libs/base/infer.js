...
infer.typeFromValue = function(value){
	if(typeof value === 'number') return Type.float
	if(typeof value === 'boolean') return Type.float
	if(typeof value === 'string') return Type.vec4
	if(typeof value === 'object'){
		...
		if(Array.isArray(value)){
			...
			if(len === 1){
				var v0 = value[0]
				if(typeof v0 === 'string') return Type.vec4
				if(typeof v0 === 'number') return Type.vec4
			}
			...
			if(len === 9) return Type.mat3
			if(len === 16) return Type.mat4
		}
...
infer.colorFromString = function(str, alpha, ar, o){
	...
	var col = str === 'random'?floor(Math.random()*16777215):infer.colormap[str]
	if(col !== undefined){
		ar[o] = (col>>16)/255
...
infer.colorFromStringPacked = function(str, alpha, ar, o){
	if(str.charCodeAt(0) === 35){ // starts with #
		var len = str.length
		if(len === 4){
			var r = hex[str.charCodeAt(1)]
...
for(var key in infer.colormap){
	infer.colormap[key.toLowerCase()] = infer.colormap[key]
}