...
impl RustCompiler {
    ...
    pub fn init(&mut self, cx: &mut Cx, text_buffers: &mut HashMap<String, TextBuffer>){
    ...
    fn clear_textbuffer_messages(&self, cx: &mut Cx, text_buffers: &mut HashMap<String, TextBuffer>){
        // clear all files we missed
        for (_, text_buffer) in text_buffers {
    ...
    pub fn export_messages(&self, cx: &mut Cx, app_global: &mut AppGlobal) {
        
        for dm in &self._draw_messages {
            
            let text_buffer = app_global.text_buffer_from_path(cx, &dm.path);
        ...
        self.clear_textbuffer_messages(cx, app_global);
        
    }
    
    pub fn handle_rust_compiler(&mut self, cx: &mut Cx, event: &mut Event, text_buffers: &mut HashMap<String, TextBuffer>){
        // do shit here
                        ...
                        SIGNAL_RUST_CHECKER | SIGNAL_RUN_OUTPUT => {
                            ...
                            if datas.len() > 0 {
                                if se.value == SIGNAL_RUST_CHECKER {
                                    ...
                                    self.export_messages(cx, app_global);
                                }
                                else {
    ...
    pub fn restart_rust_checker(&mut self, cx: &mut Cx, text_buffers: &mut HashMap<String, TextBuffer>){
