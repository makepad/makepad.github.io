...
//		'var URL = self.URL\n'+
	...
	function createOnMessage(busses, worker, inkernel){
		worker.postEntry = function(level){
			...
			if(!level) level = 0
			if((worker.postFunctions.length || worker.batchMessages.length) && level<5) worker.postEntry(level+1)
		}
	...
	// we need to define our own Promise implementation or otherwise we dont get post-hooks in workers
